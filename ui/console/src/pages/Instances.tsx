    ...
    <div class="max-w-7xl">
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">Manage your LightweightAuth data-plane deployments</p>
        </div>
        <div class="flex gap-2">
          ...
          <button
            class="inline-flex items-center gap-2 px-4 py-2.5 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50"
      ...
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {instances.isLoading && (
          <div class="px-5 py-12 text-center text-sm text-gray-400">Loading instances…</div>
        )}
        {instances.isError && (
          <div class="px-5 py-12 text-center text-sm text-red-500">
            Failed to load: {(instances.error as Error).message}
          </div>
        )}
        {instances.data && instances.data.length === 0 && (
          <div class="px-5 py-16 text-center">
            <Server size={40} class="mx-auto text-gray-300 mb-3" />
            <p class="text-sm font-medium text-gray-700">No instances yet</p>
            <p class="text-xs text-gray-500 mt-1">Create your first instance to get started</p>
          </div>
        )}
        {instances.data && instances.data.length > 0 && (
          <table class="w-full text-sm">
            <thead>
              <tr class="bg-gray-50/80 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-100">
            ...
            <tbody class="divide-y divide-gray-100">
              {instances.data.map((inst) => (
                <tr class="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td class="px-5 py-3.5 text-gray-600">{inst.cluster}</td>
                  <td class="px-5 py-3.5 text-gray-500 text-xs">{inst.namespace ?? "—"}</td>
                  <td class="px-5 py-3.5">
                    <span
                  ...
                  <td class="px-5 py-3.5 text-gray-600">{inst.status.replicas ?? "—"}</td>
                  <td class="px-5 py-3.5">
  ...
  return (
    <div class="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
      <div class="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md border border-gray-100">
        <h3 class="text-lg font-bold text-gray-900 mb-1">Link External Instance</h3>
        ...
        <form onSubmit={handleSubmit} class="flex flex-col gap-4">
          <label class="text-xs font-medium text-gray-700">
            ...
            <input
              ...
              class="mt-1.5 block w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all"
              required
            />
          </label>
          <label class="text-xs font-medium text-gray-700">
          ...
          {error() && <p class="text-red-600 text-xs font-medium">{error()}</p>}
          <div class="flex justify-end gap-3 mt-2 pt-4 border-t border-gray-100">
            <button
              type="button"
              onClick={props.onClose}
              class="px-4 py-2.5 text-sm font-medium text-gray-600 hover:text-gray-800 rounded-lg hover:bg-gray-100 transition-colors"
            >
              Cancel
            </button>
            <button
