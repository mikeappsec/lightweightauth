  ...
  return (
    <div class="max-w-7xl">
      ...
      <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm px-4 py-3 mb-4 flex items-center gap-3">
        ...
        <span class="text-xs text-gray-400 dark:text-gray-500">{alerts().length} alerts</span>
      </div>
        ...
        <div class="overflow-auto max-h-[calc(100vh-320px)]">
          <Show when={alerts().length > 0} fallback={
            <div class="py-16 text-center">
              ...
              <p class="text-sm font-medium text-gray-600 dark:text-gray-300">No active alerts</p>
                    ...
                    >
                      ...
                      <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-2 flex-wrap">
                          <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">{a.rule}</span>
                          ...
                          <span class="text-xs text-gray-400 dark:text-gray-500 font-mono ml-auto whitespace-nowrap">
                        ...
                        <div class="flex items-center gap-3 mt-1 text-xs text-gray-500 dark:text-gray-400">
                          ...
                          <Show when={a.scope["cluster"]}>
                            <span>cluster: <code class="text-gray-700 dark:text-gray-300">{a.scope["cluster"]}</code></span>
                          </Show>
                          <For each={Object.entries(a.scope).filter(([k]) => k !== "cluster")}>
                            {([k, v]) => (
                              <span>{k}: <code class="text-gray-700 dark:text-gray-300">{v}</code></span>
                            )}
                          </For>
                        </div>
                      </div>
                      <Show when={a.state === "open" || a.state === "acknowledged"}>
                        <button
                          class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium rounded-md bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 transition-colors shrink-0"
                          onClick={(e) => {
                            e.stopPropagation();
                            void handleAck(a.id);
                          }}
                      ...
                      {expanded() === a.id ? <ChevronDown size={16} class="text-gray-400 dark:text-gray-500 mt-1" /> : <ChevronRight size={16} class="text-gray-400 dark:text-gray-500 mt-1" />}
                    </button>
...
function severityBadge(s: Severity): string {
  switch (s) {
    case "critical": return "bg-red-100 dark:bg-red-500/15 text-red-700 dark:text-red-400";
    case "warning":  return "bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-400";
    default:         return "bg-blue-100 dark:bg-blue-500/15 text-blue-700 dark:text-blue-400";
  }
}

function stateBadge(s: string): string {
  switch (s) {
    case "open":         return "bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400";
    case "acknowledged": return "bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400";
    case "resolved":     return "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400";
  ...
  return (
    <div class="bg-gray-50/60 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 px-5 py-4 text-sm">
      <Show when={r()} fallback={
        ...
        <p class="text-sm text-gray-800 dark:text-gray-200 leading-relaxed font-medium mb-3">{r()!.headline}</p>
                ...
                <For each={r()!.top_contributors!.slice(0, 3)}>
                  {(c) => (
                    <li class="flex items-center gap-2 text-xs text-gray-700 dark:text-gray-300">
                      <span class="text-[10px] uppercase tracking-wide text-gray-400 dark:text-gray-500 w-24 shrink-0">{c.dimension}</span>
                      <code class="text-gray-800 dark:text-gray-200 truncate">{c.value}</code>
                      <Show when={c.share !== undefined}>
                        <span class="ml-auto text-gray-400 dark:text-gray-500 font-mono">{(c.share! * 100).toFixed(0)}%</span>
                      </Show>
                    </li>
                  )}
                </For>
            ...
            <div>
              ...
              <ul class="space-y-1">
                <For each={r()!.correlated}>
                  {(c) => (
                    <li class="flex items-center gap-2 text-xs text-gray-700 dark:text-gray-300">
                      <AlertTriangle size={11} class="text-amber-500 dark:text-amber-400" />
                      <code class="text-gray-800 dark:text-gray-200">{c.rule}</code>
                      <Show when={c.value}>
                        <span class="text-gray-400 dark:text-gray-500 font-mono">{c.value}</span>
                      </Show>
                      <Show when={c.note}>
                        <span class="ml-auto text-[10px] text-amber-600 dark:text-amber-400 italic">{c.note}</span>
                      </Show>
                    </li>
                  )}
                </For>
              ...
              <table class="w-full text-xs">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                  <For each={r()!.recent_failures}>
                    {(f) => (
                      <tr>
                        <td class="py-1.5 pr-3 font-mono text-gray-500 dark:text-gray-400 whitespace-nowrap">
                          {new Date(f.timestamp).toLocaleTimeString()}
                        </td>
                        <td class="py-1.5 pr-3 font-mono text-gray-700 dark:text-gray-300">{f.method ?? "—"}</td>
                        <td class="py-1.5 pr-3 font-mono text-gray-600 dark:text-gray-400 truncate max-w-[200px]">{f.path ?? "—"}</td>
                        <td class="py-1.5 pr-3 font-mono text-gray-500 dark:text-gray-400 truncate max-w-[160px]" title={f.subject_hash}>
                          {f.subject_hash ?? "—"}
                        </td>
                        <td class="py-1.5 pr-3 text-gray-600 dark:text-gray-400">{f.tenant ?? "—"}</td>
                        <td class="py-1.5 text-gray-500 dark:text-gray-400 truncate max-w-[200px]">{f.reason ?? "—"}</td>
                      </tr>
                    )}
                  </For>
                </tbody>
