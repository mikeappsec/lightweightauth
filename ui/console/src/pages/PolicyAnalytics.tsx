...
export default function PolicyAnalytics() {
  ...
  const columns: ColumnDef<PolicyVersionStats, any>[] = [
    {
      ...
      header: "Version",
      cell: (info) => <code class="text-gray-800 text-xs">{info.getValue() || "—"}</code>,
    },
    {
      accessorKey: "total_count",
      header: "Total",
      cell: (info) => <span class="font-mono text-gray-600">{info.getValue().toFixed(0)}</span>,
    },
    {
      accessorKey: "allow_count",
      header: "Allow",
      cell: (info) => <span class="font-mono text-green-600">{info.getValue().toFixed(0)}</span>,
    },
    {
      accessorKey: "deny_count",
      header: "Deny",
      cell: (info) => <span class="font-mono text-red-600">{info.getValue().toFixed(0)}</span>,
    },
    {
      accessorKey: "error_count",
      header: "Error",
      cell: (info) => <span class="font-mono text-orange-600">{info.getValue().toFixed(0)}</span>,
    },
    {
      accessorKey: "deny_rate",
      header: "Deny %",
      cell: (info) => (
        <span class="font-mono text-gray-600">{(info.getValue() * 100).toFixed(1)}%</span>
      ),
    },
    {
      ...
      cell: (info) => (
        <Show when={info.getValue() > 0} fallback={<span class="text-gray-300">—</span>}>
          <span class="font-mono text-amber-600">{info.getValue().toFixed(0)}</span>
        </Show>
      ),
    },
    {
  ...
  return (
    <div class="max-w-7xl">
      {/* Header */}
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Policy Analytics</h1>
                ...
                <For each={topDenyAuthors.data!.items.slice(0, 8)}>
                  {(item) => (
                    <div class="flex items-center gap-3">
                      <code class="text-xs text-gray-700 truncate w-40">{item.label}</code>
                      ...
                      <span class="text-xs font-mono text-gray-500 w-16 text-right">
